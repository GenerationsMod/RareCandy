...
public class Pipelines {
    ...
    public Pipelines(Supplier<Matrix4f> projectionMatrix) {
        for (var lightingType : LightingType.values()) {
            ...
            getPipeline(lightingType, true, projectionMatrix);
            LOGGER.info("Caching " + lightingType + " with animated = " + false);
            getPipeline(lightingType, false, projectionMatrix);
        }
    ...
    public ShaderPipeline getPipeline(LightingType lightingType, boolean animated) {
    ...
    public ShaderPipeline getPipeline(LightingType lightingType, boolean animated, Supplier<Matrix4f> projectionMatrix) {
        return SHADER_CACHE.computeIfAbsent(new ShaderSettings(lightingType, animated), shaderSettings -> {
            ...
            switch (lightingType) {
                case PBR ->
                        builder.supplyUniform("camPos", ctx -> ctx.uniform().uploadVec3f(new Vector3f(0.1f, 0.01f, -2)))
                ...
                case BASIC_FAST ->
                        builder.supplyUniform("lightPosition", ctx -> ctx.uniform().uploadVec3f(new Vector3f(0, 2, 0)))
                ...
                case GLOSSY ->
                        builder.supplyUniform("lightPosition", ctx -> ctx.uniform().uploadVec3f(new Vector3f(0, 2, -1)))
