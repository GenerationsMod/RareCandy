...
public class PixelAsset {
    ...
    public static boolean save(Path path, PixelAsset pk) {
        var files = pk.files;

        var result = JsonIo.writeAsBytes(ModelConfig.CODEC, pk.config);

        if(result.isEmpty()) return false;

        files.put("config.json", result.get());

        return save(path, files);
    }

    public static boolean save(Path path, Map<String, byte[]> files) {
