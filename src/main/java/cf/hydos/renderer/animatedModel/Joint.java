package cf.hydos.renderer.animatedModel;
 ...
 * "animatedTransform" in the {@link cf.hydos.renderer.animation.Animator} class. It also has the local bind
 * transform which is the original (no pose/com.thinmatrix.animationrenderer.animation applied) transform of the
...
public class Joint {
	...
	protected void calcInverseBindTransform(Matrix4f parentBindTransform) {
		...
		new Matrix4f(parentBindTransform).mul(localBindTransform, bindTransform);
